#!/usr/bin/env bash
# chat.sh — Send a message to Aura and collect the streaming response.
#
# Usage:
...
#   AURA_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   AURA_URL          — Unified proxy base URL (default: http://localhost:2026)
#   AURA_GATEWAY_URL  — Gateway API base URL (default: $AURA_URL)
#   AURA_LANGGRAPH_URL — LangGraph API base URL (default: $AURA_URL/api/langgraph)
#
...
GATEWAY_URL="${AURA_GATEWAY_URL:-$AURA_URL}"
LANGGRAPH_URL="${AURA_LANGGRAPH_URL:-$AURA_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: Aura is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure Aura is running. Start it with: cd <aura-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from Aura: {data_str}", file=sys.stderr)
            sys.exit(1)
