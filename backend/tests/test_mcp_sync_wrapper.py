...
from aura.mcp.tools import _make_sync_tool_wrapper, get_mcp_tools
...
def test_mcp_tool_sync_wrapper_generation():
    ...
    with (
        ...
        patch("aura.config.extensions_config.ExtensionsConfig.from_file"),
        patch("aura.mcp.tools.build_servers_config", return_value={"test-server": {}}),
        patch("aura.mcp.tools.get_initial_oauth_headers", new_callable=AsyncMock, return_value={}),
    ):
...
def test_mcp_tool_sync_wrapper_in_running_loop():
    ...
    # Test the real helper function exported from aura.mcp.tools
...
def test_mcp_tool_sync_wrapper_exception_logging():
    ...
    with patch("aura.mcp.tools.logger.error") as mock_log_error:
