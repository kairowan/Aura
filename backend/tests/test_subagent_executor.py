...
aura.subagents.executor. This test file uses delayed import via fixture to test
...
_MOCKED_MODULE_NAMES = [
    "aura.agents",
    "aura.agents.thread_state",
    "aura.agents.middlewares",
    "aura.agents.middlewares.thread_data_middleware",
    "aura.sandbox",
    "aura.sandbox.middleware",
    "aura.models",
]
...
def _setup_executor_classes():
    ...
    original_executor = sys.modules.get("aura.subagents.executor")

    # Remove mocked executor if exists (from conftest.py)
    if "aura.subagents.executor" in sys.modules:
        del sys.modules["aura.subagents.executor"]
    ...
    from aura.subagents.config import SubagentConfig
    from aura.subagents.executor import (
        SubagentExecutor,
        SubagentResult,
        SubagentStatus,
    )
    ...
    if original_executor is not None:
        sys.modules["aura.subagents.executor"] = original_executor
    elif "aura.subagents.executor" in sys.modules:
        del sys.modules["aura.subagents.executor"]
...
class TestCleanupBackgroundTask:
    ...
    def executor_module(self, _setup_executor_classes):
        ...
        from aura.subagents import executor
