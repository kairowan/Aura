...
class TestAgentConfig:
    def test_minimal_config(self):
        from aura.config.agents_config import AgentConfig
...
class TestLoadAgentConfig:
    def test_load_valid_config(self, tmp_path):
        ...
        with patch("aura.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
...
class TestLoadAgentSoul:
    def test_reads_soul_file(self, tmp_path):
        ...
        with patch("aura.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from aura.config.agents_config import AgentConfig, load_agent_soul
...
class TestListCustomAgents:
    def test_empty_when_no_agents_dir(self, tmp_path):
        with patch("aura.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            from aura.config.agents_config import list_custom_agents
...
class TestMemoryFilePath:
    def test_global_memory_path(self, tmp_path):
        ...
        import aura.agents.memory.updater as updater_mod
        from aura.config.memory_config import MemoryConfig

        with (
            patch("aura.agents.memory.updater.get_paths", return_value=_make_paths(tmp_path)),
            patch("aura.agents.memory.updater.get_memory_config", return_value=MemoryConfig(storage_path="")),
        ):
...
def agent_client(tmp_path):
    ...
    with patch("aura.config.agents_config.get_paths", return_value=paths_instance), patch("app.gateway.routers.agents.get_paths", return_value=paths_instance):
