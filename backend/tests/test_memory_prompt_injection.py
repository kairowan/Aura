...
from aura.agents.memory.prompt import _coerce_confidence, format_memory_for_injection
...
def test_format_memory_respects_budget_when_adding_facts(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aura.agents.memory.prompt._count_tokens", lambda text, encoding_name="cl100k_base": len(text))
