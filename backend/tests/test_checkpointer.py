...
import aura.config.app_config as app_config_module
from aura.agents.checkpointer import get_checkpointer, reset_checkpointer
from aura.config.checkpointer_config import (
    CheckpointerConfig,
...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("aura.agents.checkpointer.provider.get_app_config", side_effect=FileNotFoundError):
            cp = get_checkpointer()
...
# ---------------------------------------------------------------------------
# AuraClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """AuraClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            patch("aura.client.get_app_config", return_value=config_mock),
            patch("aura.client.create_agent", side_effect=fake_create_agent),
            patch("aura.client.create_chat_model", return_value=MagicMock()),
            patch("aura.client._build_middlewares", return_value=[]),
            patch("aura.client.apply_prompt_template", return_value=""),
            patch("aura.client.AuraClient._get_tools", return_value=[]),
        ):
            client = AuraClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = AuraClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)
