...
class TestCheckpointerNoneFix:
    ...
    async def test_async_make_checkpointer_returns_in_memory_saver_when_not_configured(self):
        ...
        from aura.agents.checkpointer.async_provider import make_checkpointer

        # Mock get_app_config to return a config with checkpointer=None
        mock_config = MagicMock()
        mock_config.checkpointer = None

        with patch("aura.agents.checkpointer.async_provider.get_app_config", return_value=mock_config):
    ...
    def test_sync_checkpointer_context_returns_in_memory_saver_when_not_configured(self):
        ...
        from aura.agents.checkpointer.provider import checkpointer_context

        # Mock get_app_config to return a config with checkpointer=None
        mock_config = MagicMock()
        mock_config.checkpointer = None

        with patch("aura.agents.checkpointer.provider.get_app_config", return_value=mock_config):
