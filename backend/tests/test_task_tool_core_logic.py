...
task_tool_module = importlib.import_module("aura.tools.builtins.task_tool")
...
def test_task_tool_emits_running_and_completed_events(monkeypatch):
    ...
    # task_tool lazily imports from aura.tools at call time, so patch that module-level function.
    monkeypatch.setattr("aura.tools.get_available_tools", get_available_tools)
...
def test_task_tool_returns_failed_message(monkeypatch):
    ...
    monkeypatch.setattr(task_tool_module, "get_stream_writer", lambda: events.append)
    monkeypatch.setattr(task_tool_module.time, "sleep", lambda _: None)
    monkeypatch.setattr("aura.tools.get_available_tools", lambda **kwargs: [])
