"""End-to-end tests for AuraClient.
...
Only AURA_HOME is redirected to tmp_path for filesystem isolation.
...
from aura.client import AuraClient, StreamEvent
from aura.config.app_config import AppConfig
...
def _make_e2e_config() -> AppConfig:
    ...
    return AppConfig(
        ...
        sandbox=SandboxConfig(use="aura.sandbox.local:LocalSandboxProvider"),
    )
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - AURA_HOME → tmp_path (all thread data lands in a temp dir)
    ...
    monkeypatch.setattr("aura.config.paths._paths", None)
    monkeypatch.setattr("aura.sandbox.sandbox_provider._default_sandbox_provider", None)

    # 2. Inject a clean AppConfig via the global singleton.
    config = _make_e2e_config()
    monkeypatch.setattr("aura.config.app_config._app_config", config)
    monkeypatch.setattr("aura.config.app_config._app_config_is_custom", True)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from aura.config.title_config import TitleConfig

    monkeypatch.setattr("aura.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from aura.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "aura.agents.middlewares.memory_middleware.get_memory_config",
        lambda: MemoryConfig(enabled=False),
    )

    # 5. Ensure summarization is off (default, but be explicit)
    from aura.config.summarization_config import SummarizationConfig

    monkeypatch.setattr("aura.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from aura.agents.lead_agent.agent import _build_middlewares as _original_build_middlewares
    from aura.agents.middlewares.title_middleware import TitleMiddleware
    ...
    monkeypatch.setattr("aura.client._build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A AuraClient wired to the isolated e2e_env."""
    return AuraClient(checkpointer=None, thinking_enabled=False)
...
class TestFileUploadIntegration:
    ...
    def test_upload_files(self, e2e_env, tmp_path):
        ...
        c = AuraClient(checkpointer=None, thinking_enabled=False)
        tid = str(uuid.uuid4())
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = AuraClient(checkpointer=None, plan_mode=False)
        ...
        c2 = AuraClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "aura.skills.installer.get_skills_root_path",
            lambda: skills_root,
        )
...
class TestConfigManagement:
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AURA_EXTENSIONS_CONFIG_PATH", str(config_file))

        # Force reload so the singleton picks up our test file
        from aura.config.extensions_config import reload_extensions_config
