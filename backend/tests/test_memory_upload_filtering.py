...
from aura.agents.memory.updater import _strip_upload_mentions_from_memory
from aura.agents.middlewares.memory_middleware import _filter_messages_for_memory
...
class TestFilterMessagesForMemory:
    ...
    def test_upload_with_real_question_preserves_question(self):
        ...
        msgs = [
            _human(combined),
            _ai("The file contains: Hello Aura."),
        ]
        result = _filter_messages_for_memory(msgs)
        ...
        assert result[1].content == "The file contains: Hello Aura."
...
class TestStripUploadMentionsFromMemory:
    ...
    def test_uploading_a_test_file_removed(self):
        ...
        mem = self._make_memory("User conducted a hands-on test by uploading a test file titled 'test_aura_memory_bug.txt'. User is also learning Python.")
        result = _strip_upload_mentions_from_memory(mem)
        summary = result["user"]["topOfMind"]["summary"]
        assert "test_aura_memory_bug.txt" not in summary
