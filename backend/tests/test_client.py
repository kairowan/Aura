...
from aura.config.paths import Paths
from aura.uploads.manager import PathTraversalError
...
def client(mock_app_config):
    """Create a AuraClient with mocked config loading."""
    with patch("aura.client.get_app_config", return_value=mock_app_config):
        return AuraClient()
...
class TestClientInit:
    ...
    def test_custom_params(self, mock_app_config):
        with patch("aura.client.get_app_config", return_value=mock_app_config):
            c = AuraClient(model_name="gpt-4", thinking_enabled=False, subagent_enabled=True, plan_mode=True, agent_name="test-agent")
    ...
    def test_invalid_agent_name(self, mock_app_config):
        with patch("aura.client.get_app_config", return_value=mock_app_config):
            with pytest.raises(ValueError, match="Invalid agent name"):
                AuraClient(agent_name="invalid name with spaces!")
            with pytest.raises(ValueError, match="Invalid agent name"):
                AuraClient(agent_name="../path/traversal")

    def test_custom_config_path(self, mock_app_config):
        with (
            patch("aura.client.reload_app_config") as mock_reload,
            patch("aura.client.get_app_config", return_value=mock_app_config),
        ):
            AuraClient(config_path="/tmp/custom.yaml")
            mock_reload.assert_called_once_with("/tmp/custom.yaml")

    def test_checkpointer_stored(self, mock_app_config):
        cp = MagicMock()
        with patch("aura.client.get_app_config", return_value=mock_app_config):
            c = AuraClient(checkpointer=cp)
...
class TestConfigQueries:
    ...
    def test_list_skills(self, client):
        ...
        with patch("aura.skills.loader.load_skills", return_value=[skill]) as mock_load:
    ...
    def test_list_skills_enabled_only(self, client):
        with patch("aura.skills.loader.load_skills", return_value=[]) as mock_load:
            client.list_skills(enabled_only=True)
            mock_load.assert_called_once_with(enabled_only=True)

    def test_get_memory(self, client):
        memory = {"version": "1.0", "facts": []}
        with patch("aura.agents.memory.updater.get_memory_data", return_value=memory) as mock_mem:
...
class TestExtractText:
    def test_string(self):
        assert AuraClient._extract_text("hello") == "hello"
    ...
    def test_list_text_blocks(self):
        ...
        assert AuraClient._extract_text(content) == "first\nsecond"

    def test_list_plain_strings(self):
        assert AuraClient._extract_text(["a", "b"]) == "a\nb"

    def test_empty_list(self):
        assert AuraClient._extract_text([]) == ""

    def test_other_type(self):
        assert AuraClient._extract_text(42) == "42"
...
class TestEnsureAgent:
    def test_creates_agent(self, client):
        ...
        with (
            patch("aura.client.create_chat_model"),
            patch("aura.client.create_agent", return_value=mock_agent),
            patch("aura.client._build_middlewares", return_value=[]) as mock_build_middlewares,
            patch("aura.client.apply_prompt_template", return_value="prompt") as mock_apply_prompt,
    ...
    def test_uses_default_checkpointer_when_available(self, client):
        ...
        with (
            patch("aura.client.create_chat_model"),
            patch("aura.client.create_agent", return_value=mock_agent) as mock_create_agent,
            patch("aura.client._build_middlewares", return_value=[]),
            patch("aura.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("aura.agents.checkpointer.get_checkpointer", return_value=mock_checkpointer),
        ):
            client._ensure_agent(config)
    ...
    def test_skips_default_checkpointer_when_unconfigured(self, client):
        ...
        with (
            ...
            patch("aura.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
            patch("aura.agents.checkpointer.get_checkpointer", return_value=None),
        ):
            client._ensure_agent(config)
...
class TestMcpConfig:
    def test_get_mcp_config(self, client):
        ...
        with patch("aura.client.get_extensions_config", return_value=ext_config):
    ...
    def test_update_mcp_config(self, client):
        ...
        try:
            ...
            with (
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("aura.client.get_extensions_config", return_value=current_config),
                patch("aura.client.reload_extensions_config", return_value=reloaded_config),
            ):
...
class TestSkillsManagement:
    ...
    def test_get_skill_found(self, client):
        ...
        with patch("aura.skills.loader.load_skills", return_value=[skill]):
            result = client.get_skill("test-skill")
        assert result is not None
        assert result["name"] == "test-skill"

    def test_get_skill_not_found(self, client):
        with patch("aura.skills.loader.load_skills", return_value=[]):
    ...
    def test_update_skill(self, client):
        ...
        try:
            ...
            with (
                patch("aura.skills.loader.load_skills", side_effect=[[skill], [updated_skill]]),
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=tmp_path),
                patch("aura.client.get_extensions_config", return_value=ext_config),
                patch("aura.client.reload_extensions_config"),
            ):
    ...
    def test_install_skill(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("aura.skills.installer.get_skills_root_path", return_value=skills_root):
...
class TestMemoryManagement:
    def test_reload_memory(self, client):
        ...
        with patch("aura.agents.memory.updater.reload_memory_data", return_value=data):
    ...
    def test_get_memory_config(self, client):
        config = MagicMock()
        config.enabled = True
        config.storage_path = ".aura/memory.json"
        ...
        with patch("aura.config.memory_config.get_memory_config", return_value=config):
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("aura.config.memory_config.get_memory_config", return_value=config),
            patch("aura.agents.memory.updater.get_memory_data", return_value=data),
        ):
...
class TestUploads:
    def test_upload_files(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("aura.client.get_uploads_dir", return_value=uploads_dir), patch("aura.client.ensure_uploads_dir", return_value=uploads_dir):
    ...
    def test_upload_files_reuses_single_executor_inside_event_loop(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.client.get_uploads_dir", return_value=uploads_dir),
                patch("aura.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("aura.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("aura.utils.file_conversion.convert_file_to_markdown", side_effect=fake_convert),
    ...
    def test_delete_upload_not_found(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            with patch("aura.client.get_uploads_dir", return_value=Path(tmp)):
                with pytest.raises(FileNotFoundError):
...
class TestArtifacts:
    def test_get_artifact(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("aura.client.get_paths", return_value=paths):
...
class TestScenarioConfigManagement:
    ...
    def test_mcp_update_then_skill_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aura.client.get_extensions_config", return_value=current_config),
                patch("aura.client.reload_extensions_config", return_value=reloaded_config),
            ):
            ...
            with (
                patch("aura.skills.loader.load_skills", side_effect=[[skill], [toggled]]),
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aura.client.get_extensions_config", return_value=ext_config),
                patch("aura.client.reload_extensions_config"),
            ):
...
class TestScenarioAgentRecreation:
    ...
    def test_different_model_triggers_rebuild(self, client):
        ...
        with (
            patch("aura.client.create_chat_model"),
            patch("aura.client.create_agent", side_effect=fake_create_agent),
            patch("aura.client._build_middlewares", return_value=[]),
            patch("aura.client.apply_prompt_template", return_value="prompt"),
            patch.object(client, "_get_tools", return_value=[]),
        ):
...
class TestScenarioThreadIsolation:
    ...
    def test_uploads_isolated_per_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("aura.client.get_uploads_dir", side_effect=get_dir), patch("aura.client.ensure_uploads_dir", side_effect=get_dir):
...
class TestScenarioMemoryWorkflow:
    ...
    def test_memory_full_lifecycle(self, client):
        ...
        with patch("aura.agents.memory.updater.get_memory_data", return_value=initial_data):
            mem = client.get_memory()
        assert len(mem["facts"]) == 1

        with patch("aura.agents.memory.updater.reload_memory_data", return_value=updated_data):
            refreshed = client.reload_memory()
        assert len(refreshed["facts"]) == 2

        with (
            patch("aura.config.memory_config.get_memory_config", return_value=config),
            patch("aura.agents.memory.updater.get_memory_data", return_value=updated_data),
        ):
...
class TestScenarioSkillInstallAndUse:
    ...
    def test_install_then_toggle(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("aura.skills.loader.load_skills", return_value=[installed_skill]):
                skills_result = client.list_skills()
            ...
            with (
                patch("aura.skills.loader.load_skills", side_effect=[[installed_skill], [disabled_skill]]),
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aura.client.get_extensions_config", return_value=ext_config),
                patch("aura.client.reload_extensions_config"),
            ):
    ...
    def test_upload_convertible_file_conversion_failure(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("aura.client.ensure_uploads_dir", return_value=uploads_dir),
                patch("aura.utils.file_conversion.CONVERTIBLE_EXTENSIONS", {".pdf"}),
                patch("aura.utils.file_conversion.convert_file_to_markdown", side_effect=Exception("conversion failed")),
            ):
...
class TestGatewayConformance:
    """Validate that AuraClient return dicts conform to Gateway Pydantic response models.
    ...
    def test_install_skill(self, client, tmp_path):
        ...
        with patch("aura.skills.installer.get_skills_root_path", return_value=tmp_path):
            result = client.install_skill(archive)
    ...
    def test_update_mcp_config(self, client, tmp_path):
        ...
        with (
            patch("aura.client.get_extensions_config", return_value=ext_config),
            patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
            patch("aura.client.reload_extensions_config", return_value=ext_config),
        ):
    ...
    def test_get_memory_config(self, client):
        ...
        mem_cfg.storage_path = ".aura/memory.json"
        ...
        with patch("aura.config.memory_config.get_memory_config", return_value=mem_cfg):
            result = client.get_memory_config()
    ...
    def test_get_memory_status(self, client):
        ...
        with (
            patch("aura.config.memory_config.get_memory_config", return_value=mem_cfg),
            patch("aura.agents.memory.updater.get_memory_data", return_value=memory_data),
        ):
            result = client.get_memory_status()
...
class TestInstallSkillSecurity:
    ...
    def test_zip_bomb_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            from aura.skills import installer as _installer
            ...
            with (
                patch("aura.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("aura.skills.installer.safe_extract_skill_archive", side_effect=patched_extract),
            ):
    ...
    def test_invalid_skill_name_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("aura.skills.installer._validate_skill_frontmatter", return_value=(True, "OK", "../evil")),
            ):
    ...
    def test_existing_skill_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("aura.skills.installer._validate_skill_frontmatter", return_value=(True, "OK", "dupe-skill")),
            ):
    ...
    def test_invalid_frontmatter_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.skills.installer.get_skills_root_path", return_value=skills_root),
                patch("aura.skills.installer._validate_skill_frontmatter", return_value=(False, "Missing name field", "")),
            ):
...
class TestAtomicWriteJson:
    def test_temp_file_cleaned_on_serialization_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with pytest.raises(TypeError):
                AuraClient._atomic_write_json(target, bad_data)
    ...
    def test_happy_path_writes_atomically(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            AuraClient._atomic_write_json(target, data)
...
class TestConfigUpdateErrors:
    def test_update_mcp_config_no_config_file(self, client):
        ...
        with patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=None):
    ...
    def test_update_skill_no_config_file(self, client):
        ...
        with (
            patch("aura.skills.loader.load_skills", return_value=[skill]),
            patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=None),
        ):
            with pytest.raises(FileNotFoundError, match="Cannot locate"):
    ...
    def test_update_skill_disappears_after_write(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.skills.loader.load_skills", side_effect=[[skill], []]),
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aura.client.get_extensions_config", return_value=ext_config),
                patch("aura.client.reload_extensions_config"),
            ):
...
class TestSerializeMessage:
    def test_system_message(self):
        ...
        result = AuraClient._serialize_message(msg)
...
class TestBugListUploadsDeadCode:
    ...
    def test_list_uploads_on_fresh_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("aura.uploads.manager.get_paths", return_value=mock_paths):
...
class TestBugAgentInvalidationInconsistency:
    ...
    def test_update_mcp_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aura.client.get_extensions_config", return_value=current_config),
                patch("aura.client.reload_extensions_config", return_value=reloaded),
            ):
    ...
    def test_update_skill_resets_config_key(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("aura.skills.loader.load_skills", side_effect=[[skill], [updated]]),
                patch("aura.client.ExtensionsConfig.resolve_config_path", return_value=config_file),
                patch("aura.client.get_extensions_config", return_value=ext_config),
                patch("aura.client.reload_extensions_config"),
            ):
