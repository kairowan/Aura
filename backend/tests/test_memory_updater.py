...
from aura.agents.memory.prompt import format_conversation_for_update
from aura.agents.memory.updater import MemoryUpdater, _extract_text
...
def test_apply_updates_skips_existing_duplicate_and_preserves_removals() -> None:
    ...
    with patch(
        "aura.agents.memory.updater.get_memory_config",
...
def test_apply_updates_skips_same_batch_duplicates_and_keeps_source_metadata() -> None:
    ...
    update_data = {
        "newFacts": [
            ...
            {"content": "User works on Aura", "category": "context", "confidence": 0.87},
        ],
    }
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_string_response_parses(self):
        ...
        with (
            ...
            patch("aura.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True)),
            patch("aura.agents.memory.updater.get_memory_data", return_value=_make_memory()),
            patch("aura.agents.memory.updater._save_memory_to_file", return_value=True),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "Hello"
