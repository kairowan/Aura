...
from aura.config.acp_config import ACPAgentConfig
from aura.config.extensions_config import ExtensionsConfig, McpServerConfig, set_extensions_config
from aura.tools.builtins.invoke_acp_agent_tool import (
    _build_mcp_servers,
...
def test_build_mcp_servers_filters_disabled_and_maps_transports():
    ...
    monkeypatch.setattr(
        "aura.config.extensions_config.ExtensionsConfig.from_file",
...
def test_get_available_tools_includes_invoke_acp_agent_when_agents_configured(monkeypatch):
    ...
    monkeypatch.setattr("aura.tools.tools.get_app_config", lambda: fake_config)
    monkeypatch.setattr(
        "aura.config.extensions_config.ExtensionsConfig.from_file",
        classmethod(lambda cls: ExtensionsConfig(mcp_servers={}, skills={})),
    )
