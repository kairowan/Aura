...
def test_replace_virtual_path_maps_virtual_root_and_subpaths() -> None:
    assert Path(replace_virtual_path("/mnt/user-data/workspace/a.txt", _THREAD_DATA)).as_posix() == "/tmp/aura/threads/t1/user-data/workspace/a.txt"
    assert Path(replace_virtual_path("/mnt/user-data", _THREAD_DATA)).as_posix() == "/tmp/aura/threads/t1/user-data"
...
def test_mask_local_paths_in_output_hides_host_paths() -> None:
    output = "Created: /tmp/aura/threads/t1/user-data/workspace/result.txt"
    masked = mask_local_paths_in_output(output, _THREAD_DATA)

    assert "/tmp/aura/threads/t1/user-data" not in masked
...
def test_mask_local_paths_in_output_hides_skills_host_paths() -> None:
    ...
    with (
        patch("aura.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("aura.sandbox.tools._get_skills_host_path", return_value="/home/user/aura/skills"),
    ):
        output = "Reading: /home/user/aura/skills/public/bootstrap/SKILL.md"
        masked = mask_local_paths_in_output(output, _THREAD_DATA)

        assert "/home/user/aura/skills" not in masked
...
def test_validate_local_tool_path_rejects_traversal_in_skills() -> None:
    ...
    with patch("aura.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"):
        with pytest.raises(PermissionError, match="path traversal"):
...
def test_validate_local_tool_path_rejects_none_thread_data() -> None:
    ...
    from aura.sandbox.exceptions import SandboxRuntimeError
...
def test_resolve_skills_path_resolves_correctly() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/aura/skills/public/bootstrap/SKILL.md"
...
def test_resolve_skills_path_resolves_root() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/aura/skills"
...
def test_resolve_skills_path_raises_when_not_configured() -> None:
    ...
    with (
        patch("aura.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("aura.sandbox.tools._get_skills_host_path", return_value=None),
    ):
...
def test_replace_virtual_paths_in_command_replaces_skills_paths() -> None:
    ...
    with (
        ...
        assert "/home/user/aura/skills/public/bootstrap/SKILL.md" in result
...
def test_replace_virtual_paths_in_command_replaces_both() -> None:
    ...
    with (
        patch("aura.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("aura.sandbox.tools._get_skills_host_path", return_value="/home/user/skills"),
    ):
...
def test_validate_local_tool_path_skills_custom_container_path() -> None:
    ...
    with patch("aura.sandbox.tools._get_skills_container_path", return_value="/custom/skills"):
        # Should not raise
        validate_local_tool_path(
...
def test_resolve_acp_workspace_path_resolves_correctly(tmp_path: Path) -> None:
    ...
    acp_dir.mkdir()
    with patch("aura.sandbox.tools._get_acp_workspace_host_path", return_value=str(acp_dir)):
...
def test_resolve_acp_workspace_path_raises_when_not_available() -> None:
    ...
    with patch("aura.sandbox.tools._get_acp_workspace_host_path", return_value=None):
...
def test_replace_virtual_paths_in_command_replaces_acp_workspace() -> None:
    ...
    acp_host = "/home/user/.aura/acp-workspace"
    with patch("aura.sandbox.tools._get_acp_workspace_host_path", return_value=acp_host):
        ...
        assert "/tmp/aura/threads/t1/user-data/outputs/hello.py" in result
