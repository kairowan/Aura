# Contributing to Aura Backend

Thank you for your interest in contributing to Aura! This document provides guidelines and instructions for contributing to the backend codebase.
...
```python
import pytest
from aura.models.factory import create_chat_model
...
1. Create tool in `packages/harness/aura/tools/builtins/` or `packages/harness/aura/community/`:

```python
# packages/harness/aura/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: aura.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/aura/agents/middlewares/`:

```python
# packages/harness/aura/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/aura/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/aura/config/app_config.py` with new fields
