...
Aura 后端当前是一个单一 Python 包（`src.*`），包含了从底层 agent 编排到上层用户产品的所有代码。随着项目发展，这种结构带来了几个问题：
...
本文档提出将后端拆分为两部分：**aura-harness**（可发布的 agent 框架包）和 **app**（不打包的用户产品代码）。
...
**Harness 是一个可发布的 Python 包**（`aura-harness`），可以独立安装和使用。
...
**App 不打包、不发布**，它是 Aura 项目内部的应用代码，直接运行。
...
├── packages/
│   └── harness/
│       ├── pyproject.toml          # aura-harness 包定义
│       └── aura/               # Python 包根（import 前缀: aura.*）
│           ├── __init__.py
│           ├── config/
│           ├── reflection/
│           ├── utils/
...
from aura.config.extensions_config import get_extensions_config
```
...
from aura.agents.lead_agent.agent import make_lead_agent
from aura.models import create_chat_model
from aura.config import get_app_config
...
from aura.skills import load_skills
from aura.agents.memory.updater import get_memory_data
...
|------|------------------------|--------------------------|
| 命名空间 | 需要 pkgutil `extend_path` 合并，或独立前缀 | 天然独立，`app.*` vs `aura.*` |
| 发布需求 | 没有——App 是项目内部代码 | 不需要 pyproject.toml |
| 复杂度 | 需要管理两个包的构建、版本、依赖声明 | 直接运行，零额外配置 |
| 运行方式 | `pip install aura-app` | `PYTHONPATH=. uvicorn app.gateway.app:app` |

App 的唯一消费者是 Aura 项目自身，没有独立发布的需求。放在 `backend/app/` 下作为普通 Python 包，通过 `PYTHONPATH` 或 editable install 让 Python 找到即可。
...
**解决方案**：将该函数提取到 `aura/skills/validation.py`。这是一个纯逻辑函数（解析 YAML frontmatter、校验字段），与 FastAPI 无关。
...
**解决方案**：将它们提取到 `aura/utils/file_conversion.py`。仅依赖 `markitdown` + `pathlib`，是通用工具函数。
...
```json
{
  ...
  "graphs": {
    "lead_agent": "aura.agents:make_lead_agent"
  },
  "checkpointer": {
    "path": "./packages/harness/aura/agents/checkpointer/async_provider.py:make_checkpointer"
  }
}
```
...
```bash
# serve.sh / Makefile
# PYTHONPATH 包含 backend/ 根目录，使 app.* 和 aura.* 都能被找到
...
Dockerfile 中的 module 引用从 `src.` 改为 `aura.` / `app.`，`COPY` 命令需覆盖 `packages/` 和 `app/` 目录。
...
2. `git mv` 将 harness 相关模块从 `src/` 移入 `packages/harness/aura/`
3. `git mv` 将 app 相关模块从 `src/` 移入 `app/`
4. 全局替换 import：
   - harness 模块：`src.*` → `aura.*`（所有 `.py` 文件、`langgraph.json`、测试、文档）
...
| `config.yaml` 中的 `use` 字段引用旧路径 | 运行时模块解析失败 | `config.yaml` 中的 `use` 字段同步更新为 `aura.*` |
| 测试中 `sys.path` 混乱 | 测试失败 | 用 editable install（`uv sync`）确保 aura 可导入，`conftest.py` 中添加 `app/` 到 `sys.path` |

## 8. 未来演进

- **独立发布**：harness 可以发布到内部 PyPI，让其他项目直接 `pip install aura-harness`
- **插件化 App**：不同的 app（web、CLI、bot）可以各自独立，都依赖同一个 harness
- **更细粒度拆分**：如果 harness 内部模块继续增长，可以进一步拆分（如 `aura-sandbox`、`aura-mcp`）
