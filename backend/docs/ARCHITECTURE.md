...
This document provides a comprehensive overview of the Aura backend architecture.
...
**Entry Point**: `packages/harness/aura/agents/lead_agent/agent.py:make_lead_agent`
...
```json
{
  "agent": {
    "type": "agent",
    "path": "aura.agents:make_lead_agent"
  }
}
```
...
- `threads.py` - `/api/threads/{id}` - Local Aura thread data cleanup after LangGraph deletion
- `artifacts.py` - `/api/threads/{id}/artifacts` - Artifact serving
- `suggestions.py` - `/api/threads/{id}/suggestions` - Follow-up suggestion generation

The web conversation delete flow is now split across both backend surfaces: LangGraph handles `DELETE /api/langgraph/threads/{thread_id}` for thread state, then the Gateway `threads.py` router removes Aura-managed filesystem data via `Paths.delete_thread_dir()`.
...
│  (packages/harness/aura/sandbox/local.py) │              │  (packages/harness/aura/community/)       │
│                         │              │                         │
...
|-------------|---------------|
| `/mnt/user-data/workspace` | `backend/.aura/threads/{thread_id}/user-data/workspace` |
| `/mnt/user-data/uploads` | `backend/.aura/threads/{thread_id}/user-data/uploads` |
| `/mnt/user-data/outputs` | `backend/.aura/threads/{thread_id}/user-data/outputs` |
| `/mnt/skills` | `aura/skills/` |
...
│  (packages/harness/aura/tools/)       │  │  (config.yaml)      │  │  (extensions.json)  │
├─────────────────────┤  ├─────────────────────┤  ├─────────────────────┤
...
└─────────────────────┘  └─────────────────────┘  └─────────────────────┘
           ...
           └───────────────────────┴───────────────────────┘
                      ...
                      ┌─────────────────────────┐
                      │   get_available_tools() │
                      │   (packages/harness/aura/tools/__init__)  │
                      └─────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Model Factory                                   │
│                     (packages/harness/aura/models/factory.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          MCP Integration                                 │
│                        (packages/harness/aura/mcp/manager.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Skills System                                   │
│                       (packages/harness/aura/skills/loader.py)                             │
└─────────────────────────────────────────────────────────────────────────┘
...
2. Gateway receives file
   - Validates file
   - Stores in .aura/threads/{thread_id}/user-data/uploads/
...
3. Gateway removes local Aura-managed files
   - Deletes .aura/threads/{thread_id}/ recursively
