...
Aura now supports Apple Container as the preferred container runtime on macOS, with automatic fallback to Docker.

## Overview

Starting with this version, Aura automatically detects and uses Apple Container on macOS when available, falling back to Docker when:
...
The implementation is in `backend/packages/harness/aura/community/aio_sandbox/aio_sandbox_provider.py`:
...
```
INFO:aura.community.aio_sandbox.aio_sandbox_provider:Detected Apple Container: container version 0.1.0
INFO:aura.community.aio_sandbox.aio_sandbox_provider:Starting sandbox container using container: ...
```

Or for Docker:
```
INFO:aura.community.aio_sandbox.aio_sandbox_provider:Apple Container not available, falling back to Docker
INFO:aura.community.aio_sandbox.aio_sandbox_provider:Starting sandbox container using docker: ...
```
...
**Usage:**
```bash
# Clean up all Aura sandbox containers
./scripts/cleanup-containers.sh aura-sandbox
