...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in Aura 2.0.
...
### Agent Module
- **Location**: `packages/harness/aura/agents/lead_agent/agent.py`
...
Aura uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall Aura prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with Aura's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/aura/backend/packages/harness/aura/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom Aura-style prompts**
...
- Custom prompts emphasize the same principles as Aura's main system prompt (clarity, action-oriented, critical rules)
