...
Aura 后端提供了完整的文件上传功能，支持多文件上传，并自动将 Office 文档和 PDF 转换为 Markdown 格式。
  ...
  "files": [
    {
      ...
      "size": 1234567,
      "path": ".aura/threads/{thread_id}/user-data/uploads/document.pdf",
      "virtual_path": "/mnt/user-data/uploads/document.pdf",
      "artifact_url": "/api/threads/{thread_id}/artifacts/mnt/user-data/uploads/document.pdf",
      "markdown_file": "document.md",
      "markdown_path": ".aura/threads/{thread_id}/user-data/uploads/document.md",
...
- 实际存储：`backend/.aura/threads/{thread_id}/user-data/uploads/document.pdf`
- 前端访问：`/api/threads/{thread_id}/artifacts/mnt/user-data/uploads/document.pdf`（HTTP URL）

上传流程采用“线程目录优先”策略：
- 先写入 `backend/.aura/threads/{thread_id}/user-data/uploads/` 作为权威存储
...
2. **Uploads Middleware** (`packages/harness/aura/agents/middlewares/uploads_middleware.py`)
   - 在每次 Agent 请求前注入文件列表
   - 自动生成格式化的文件列表消息
...
2. 检查 thread_id 是否正确
3. 确认文件确实已上传到 `backend/.aura/threads/{thread_id}/user-data/uploads/`
