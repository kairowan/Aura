...
This guide explains how to configure Aura for your environment.
...
- Claude Code OAuth (`aura.models.claude_provider:ClaudeChatModel`)
- Codex CLI (`aura.models.openai_codex_provider:CodexChatModel`)
...
- On macOS, Aura does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Standard `langchain_openai:ChatOpenAI` silently drops `thought_signature` when serialising messages.  Use `aura.models.patched_openai:PatchedChatOpenAI` instead — it re-injects the tool-call signatures (sourced from `AIMessage.additional_kwargs["tool_calls"]`) into every outgoing payload:

```yaml
models:
  - name: gemini-2.5-pro-thinking
    display_name: Gemini 2.5 Pro (Thinking)
    use: aura.models.patched_openai:PatchedChatOpenAI
...
tools:
  - name: web_search
    group: web
    use: aura.community.tavily.tools:web_search_tool
    max_results: 5
...
Aura supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:

**Local Execution** (runs sandbox code directly on the host machine):
```yaml
sandbox:
   use: aura.sandbox.local:LocalSandboxProvider # Local execution
```

**Docker Execution** (runs sandbox code in isolated Docker containers):
```yaml
sandbox:
   use: aura.community.aio_sandbox:AioSandboxProvider # Docker-based sandbox
```
...
```yaml
sandbox:
   use: aura.community.aio_sandbox:AioSandboxProvider
   provisioner_url: http://provisioner:8002
```

When using Docker development (`make docker-start`), Aura starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
```yaml
sandbox:
  use: aura.sandbox.local:LocalSandboxProvider
```
...
**How Skills Work**:
- Skills are stored in `aura/skills/{public,custom}/`
...
Aura supports environment variable substitution using the `$` prefix:
...
- `AURA_CONFIG_PATH` - Custom config file path

## Configuration Location

The configuration file should be placed in the **project root directory** (`aura/config.yaml`), not in the backend directory.
...
2. Path from `AURA_CONFIG_PATH` environment variable
3. `config.yaml` in current working directory (typically `backend/` when running)
4. `config.yaml` in parent directory (project root: `aura/`)
...
- Ensure `config.yaml` exists in the **project root** directory (`aura/config.yaml`)
- The backend searches parent directory by default, so root location is preferred
- Alternatively, set `AURA_CONFIG_PATH` environment variable to custom location
...
### "Skills not loading"
- Check that `aura/skills/` directory exists
