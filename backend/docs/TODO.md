...
- [ ] Optimize async concurrency in agent hot path (IM channels multi-task scenario)
  - Replace `time.sleep(5)` with `asyncio.sleep()` in `packages/harness/aura/tools/builtins/task_tool.py` (subagent polling)
  - Replace `subprocess.run()` with `asyncio.create_subprocess_shell()` in `packages/harness/aura/sandbox/local/local_sandbox.py`
