...
Quick setup instructions for Aura.

## Configuration Setup

Aura uses a YAML configuration file that should be placed in the **project root directory**.
...
4. **Verify configuration**:
   ```bash
   cd backend
   python -c "from aura.config import get_app_config; print('✓ Config loaded:', get_app_config().models[0].name)"
   ```

## Important Notes

- **Location**: `config.yaml` should be in `aura/` (project root), not `aura/backend/`
...
1. `AURA_CONFIG_PATH` environment variable (if set)
2. `backend/config.yaml` (current directory when running from backend/)
3. `aura/config.yaml` (parent directory - **recommended location**)

**Recommended**: Place `config.yaml` in project root (`aura/config.yaml`).

## Sandbox Setup (Optional but Recommended)

If you plan to use Docker/Container-based sandbox (configured in `config.yaml` under `sandbox.use: aura.community.aio_sandbox:AioSandboxProvider`), it's highly recommended to pre-pull the container image:
...
cd aura/backend
python -c "from aura.config.app_config import AppConfig; print(AppConfig.resolve_config_path())"
```
