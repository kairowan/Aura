...
The simplest option. Ships with Aura. Block or allow tools by name. No external packages, no passport, no network.
...
guardrails:
  enabled: true
  provider:
    use: aura.guardrails.builtin:AllowlistProvider
    config:
...
aport setup --framework aura
```

This creates:
- `~/.aport/aura/config.yaml` -- evaluator config (local or API mode)
- `~/.aport/aura/aport/passport.json` -- OAP passport with capabilities and limits
...
Any provider that accepts `framework` as a kwarg and implements `evaluate`/`aevaluate` works. The OAP standard defines the passport format and decision codes; Aura doesn't care which provider reads them.
...
A custom OAP provider can implement any evaluation strategy -- the Aura middleware doesn't care how the provider reaches its decision.

**Try it:**
1. Install and set up as above
2. Start Aura and ask: "Create a file called test.txt with content hello"
...
class MyGuardrailProvider:
    ...
    def evaluate(self, request):
        from aura.guardrails.provider import GuardrailDecision, GuardrailReason
...
2. Add the config
3. Start Aura and ask: "Use bash to delete test.txt"
...
Aura loads providers via `resolve_variable()` -- the same mechanism used for models, tools, and sandbox providers. The `use:` field is a Python class path: `package.module:ClassName`.

The provider is instantiated with `**config` kwargs if `config:` is set, plus `framework="aura"` is always injected. Accept `**kwargs` to stay forward-compatible:

```python
class YourProvider:
    def __init__(self, framework: str = "generic", **kwargs):
        # framework="aura" tells you which config dir to use
        ...
```
...
```
packages/harness/aura/guardrails/
...
packages/harness/aura/config/
    guardrails_config.py     # GuardrailsConfig Pydantic model + singleton

packages/harness/aura/agents/middlewares/
