...
Gateway (`app/gateway/routers/skills.py`, `uploads.py`) and Client (`aura/client.py`) each independently implement the same business logic:
...
app.gateway.routers.uploads ──┤── calls ──→  aura.skills.installer
aura.client             ──┘              aura.uploads.manager
```

- Shared modules live in the harness layer (`aura.*`), pure business logic, no FastAPI dependency
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `AuraClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `AuraClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.
