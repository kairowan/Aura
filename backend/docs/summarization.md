...
Aura includes automatic conversation summarization to handle long conversations that approach model token limits. When enabled, the system automatically condenses older messages while preserving recent context.
...
- **Configuration**: `packages/harness/aura/config/summarization_config.py`
- **Integration**: `packages/harness/aura/agents/lead_agent/agent.py`
