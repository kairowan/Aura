...
class Paths:
    """
    Centralized path configuration for Aura application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. AURA_HOME environment variable
        3. Local dev fallback: cwd/.aura  (when cwd is the backend/ dir)
        4. Default: $HOME/.aura
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set AURA_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("AURA_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("AURA_HOME"):
