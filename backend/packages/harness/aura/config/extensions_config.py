...
class ExtensionsConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path | None:
        ...
        2. If provided `AURA_EXTENSIONS_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("AURA_EXTENSIONS_CONFIG_PATH"):
            path = Path(os.getenv("AURA_EXTENSIONS_CONFIG_PATH"))
            if not path.exists():
                raise FileNotFoundError(f"Extensions config file specified by environment variable `AURA_EXTENSIONS_CONFIG_PATH` not found at {path}")
            return path
        else:
