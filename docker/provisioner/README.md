# Aura Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the Aura backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `aura` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `aura` | Kubernetes namespace for sandbox resources |
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `aura` namespace
     - Create/read/delete Services in the `aura` namespace
     - Read Namespaces (to create `aura` if missing)
...
docker compose -p aura-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec aura-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec aura-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec aura-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n aura -l sandbox-id=test-001

# Delete sandbox
docker exec aura-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec aura-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec aura-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec aura-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.aura/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n aura`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n aura`
...
3. **Network Isolation**: Sandbox Pods run in the `aura` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
