...
### 一、后端配置层 (`backend/packages/harness/aura/config/extensions_config.py`)
...
### 二、后端技能加载器 (`backend/packages/harness/aura/skills/loader.py`)
...
- **后端文件**: 3 个文件修改
  - `backend/packages/harness/aura/config/extensions_config.py`: +1 方法，修改 1 方法
  - `backend/packages/harness/aura/skills/loader.py`: +重复检查逻辑
