...
if [ -n "$AURA_CONFIG_PATH" ] && [ -f "$AURA_CONFIG_PATH" ]; then
    CONFIG="$AURA_CONFIG_PATH"
...
MIGRATIONS = {
    1: {
        'description': 'Rename src.* module paths to aura.*',
        'replacements': [
            ('src.community.', 'aura.community.'),
            ('src.sandbox.', 'aura.sandbox.'),
            ('src.models.', 'aura.models.'),
            ('src.tools.', 'aura.tools.'),
        ],
    },
