...
COMPOSE_CMD="docker compose -p aura-dev -f docker-compose-dev.yaml"
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"aura.sandbox.local:LocalSandboxProvider"* ]]; then
        echo "local"
    elif [[ "$sandbox_use" == *"aura.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
init() {
    echo "=========================================="
    echo "  Aura Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting Aura Docker Development"
    echo "=========================================="
    echo ""
    ...
    # Set AURA_ROOT for provisioner if not already set
    if [ -z "$AURA_ROOT" ]; then
        export AURA_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting AURA_ROOT=$AURA_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting Aura.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
...
stop() {
    # AURA_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" aura-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting Aura Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "Aura Docker Management Script"
    echo ""
