# Copilot Onboarding Instructions for Aura
...
Aura is a full-stack "super agent harness".
...
- `backend/packages/harness/aura/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/aura/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/aura/subagents/` - subagent registry/execution
- `backend/packages/harness/aura/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`aura.agents:make_lead_agent`)
